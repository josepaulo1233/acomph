...
dia_1dia=(now-timedelta(days=1)).day
mes_1dia=(now-timedelta(days=1)).month
ano_1dia=(now-timedelta(days=1)).year

# arquivos necessários
file_acomph = f"./acomph/ACOMPH_{str(dia).zfill(2)}.{str(mes).zfill(2)}.{ano}.xls"
file_psat = f"./psat/psat_{str(dia_1dia).zfill(2)}{str(mes_1dia).zfill(2)}{ano_1dia}.txt"
    ...
    def _single(ax):
        if orient == "v":
            for p in ax.patches:
                ...
                ax.text(_x, _y, value, ha="center", size=17.5, color='black') 
        elif orient == "h":
            for p in ax.patches:
...
if os.path.exists(file_acomph) == True and os.path.exists(file_psat) == True and os.path.exists(prods) == True:
    ...
    for bacia in bacias: 
        ...
        if os.path.exists(file_psat) == True:
            ...
            plt.xticks(rotation=90)
            plt.title(f'Série temporal da Precipitação para a bacia do rio {nome_da_bacia}', loc='left', fontsize = 25, fontweight="bold")
            plt.title(f'Última atualização PSAT - {file_acomph[16:26]}', loc='right', fontsize = 25, fontweight="bold")
            plt.yticks(fontsize=20)
            plt.xticks(fontsize=20)
        ...
        if os.path.exists(file_acomph) == True and os.path.exists(file_psat) == True:
            ...
            ax2.set_ylabel('\nENA [WMmed]', fontsize=20)
            ax.grid(False)
            ax2.grid(False)
            plt.title(f'Série temporal ENA x PSAT para a bacia do rio {nome_da_bacia}', loc='left', fontsize = 25, fontweight="bold")
            plt.title(f'Última atualização - {file_acomph[16:26]}', loc='right', fontsize = 25, fontweight="bold")
            fig.legend(bbox_to_anchor=(1.17,1), bbox_transform=ax.transAxes, fancybox=True, shadow=True, fontsize=20)
            os.makedirs(path_dest, exist_ok=True)
